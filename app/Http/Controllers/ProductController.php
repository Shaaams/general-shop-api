        ...
        $product->category_id    = intval($request->input('product_category'));
            ...
            if($request->has('options'))
            {
                ...
                foreach ($options as $option) {
                    $optionActuals = $request->input($option);
                    $optionActualArray[$option] =[];

                    if ($optionActuals)
                    {

                            foreach ($optionActuals as $optionActual)
                            {
                                array_push($optionActualArray[$option], $optionActual );
                            }

                    }
                }
                //dd($optionActualArray);

                $product->options = json_encode($optionActualArray);
            }
